...
We will use some data that have been simulated to demonstrate many of the problems we have seen already. Each row represents a different patient in a fictitious study and can be downloaded from the [course website](patient-data.csv). (Right-click and `Save Link as....`) 
...
- Change the value to `FALSE`. Clicking ***Apply to all identical cells*** will change all occurrences of `No` to `FALSE`.
...
We can now explicitly convert the Height values into numbers

- Select ***Edit cells*** -> ***Common Transformations*** -> ***To number***

It is now possible to facet the column by number rather than text. This allows potential outlier values to be identified
...
- Split the `Birth` column into Year, Month and Day
- Tidy up the following columns ready for analysis:-
  + `Weight`
  + `Grade_Level` 
  + `Count`
